...
def create_staff():
    ...
    email    = d.get("email", "").strip().lower()
    password = d.get("password", "")
    agency_email = d.get("agencyEmail", "").strip()
    ...
    users_col().insert_one({
        "name": name, "email": email, "password": _hash(password),
        "role": "staff", "agencyEmail": agency_email,
        "createdAt": datetime.datetime.utcnow()
    })
    return jsonify({"message": f"Staff '{name}' created successfully"}), 201
