...
def agency_complaints():
    ...
    if g: return g
    claims       = get_jwt()
    agency_email = claims.get("email")      # staff email = agency identifier
    status       = request.args.get("status")

    # For staff, show complaints assigned to their agencyEmail
    query = {"agencyEmail": agency_email}
...
def staff_stats():
    g = _staff_guard()
    if g: return g
    ae = get_jwt().get("email")
    return jsonify({
        "total":    complaints_col().count_documents({"agencyEmail": ae}),
        "pending":  complaints_col().count_documents({"agencyEmail": ae, "status": "Pending"}),
        "cleaned":  complaints_col().count_documents({"agencyEmail": ae, "status": "Cleaned"}),
        "rejected": complaints_col().count_documents({"agencyEmail": ae, "status": "Rejected"}),
    }), 200
