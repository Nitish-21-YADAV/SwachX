...
function StatCard({ label, value, color }) {
  return (
    <div className="card card-glow p-5">
      <div style={{fontSize:'2.4rem',fontFamily:'Syne,sans-serif',fontWeight:800,
                   color:color||'var(--acid)',lineHeight:1}}>{value}</div>
      <div style={{color:'var(--text-2)',fontSize:'11px',fontFamily:'Syne,sans-serif',fontWeight:700,
                   letterSpacing:'0.8px',textTransform:'uppercase',marginTop:'6px'}}>{label}</div>
    </div>
  )
}
  ...
  return (
    <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.85)',zIndex:1000,
      ...
      <div style={{position:'relative',maxWidth:'90vw',maxHeight:'90vh'}} onClick={e=>e.stopPropagation()}>
        <button onClick={onClose} style={{position:'absolute',top:'-40px',right:'0',background:'none',border:'none',
                                          cursor:'pointer',color:'white'}}><X size={24}/></button>
          ...
          <>
            ...
            <div className="card" style={{overflowX:'auto'}}>
              <table className="table">
                <thead>
                  <tr>
                    {['Image','Complaint No.','User','Waste Type','Agency','Pincode','Status','Timestamp','SSIM','Actions'].map(h=>(
                      <th key={h}>{h}</th>
                    ))}
                  </tr>
                </thead>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="card card-glow p-6">
              ...
              <form onSubmit={createStaff}>
                {[['name','Name','text','Full name'],['email','Email','email','staff@agency.gov.in'],['password','Password','password','Minimum 6 chars']].map(([k,lab,type,ph])=>(
                  <div key={k} className="mb-4">
                    <label className="label">{lab}</label>
                    <input className="input" type={type} value={newStaff[k]} placeholder={ph} required
                ...
                <div className="mb-4">
                  ...
                  <input className="input" type="email" placeholder="agency@bmc.gov.in"
                         value={newStaff.agencyEmail||''} onChange={e=>setNewStaff(p=>({...p,agencyEmail:e.target.value}))} />
