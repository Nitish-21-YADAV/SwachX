...
export default function NewComplaint() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      fd.append('pincode',   meta?.resolvedPincode || meta?.pincode || '')
      ...
      setDone(data)
      toast.success(`Complaint ${data.complaintNumber} registered!`)
          ...
          {STEP_LABELS.map((lab,i) => (
            <div key={i} className="flex items-center gap-2">
              <div className="flex items-center gap-2">
                ...
                <span style={{ fontSize:'13px', fontFamily:'Syne,sans-serif', fontWeight:600,
                               color: i===step ? 'var(--text-1)' : 'var(--text-3)' }}>{lab}</span>
              </div>
        ...
        {step === 0 && (
          <div className="animate-fade-up">
            ...
            <p style={{color:'var(--text-2)',fontSize:'14px',marginBottom:'24px'}}>
              GPS and pincode will be extracted automatically from image EXIF metadata.
            </p>
            ...
            {meta && !metaLoading && (
              <div className="card p-5 mt-4">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="section-title" style={{fontSize:'13px'}}>Extracted Metadata</span>
                </div>
                <div className="grid grid-cols-2 gap-3">
                  {[
                    ...
                    <div key={k} className="p-3 rounded-lg" style={{background:'rgba(255,255,255,0.02)',border:'1px solid var(--border)'}}>
                      ...
                      <div style={{color:'var(--text-1)',fontSize:'13px',marginTop:'3px',fontFamily:'JetBrains Mono,monospace'}}>{String(v)}</div>
                    </div>
                  ))}
                </div>
              </div>
          ...
          <div className="animate-fade-up">
            ...
            <div className="card card-glow p-6 mb-4">
              ...
              {[
                ...
                <div key={k} className="mb-2 rounded-xl overflow-hidden"
                     style={{border:'1px solid var(--border)'}}>
                  <button onClick={()=>toggle(k)}
                          className="w-full flex items-center justify-between p-3"
                          style={{background:'rgba(255,255,255,0.02)',border:'none',cursor:'pointer',textAlign:'left'}}>
                  ...
                  {expanded[k] && (
                    <div className="p-3 pt-0" style={{color:'var(--text-2)',fontSize:'13px',lineHeight:1.7,padding:'12px'}}>
                      {val}
                    </div>
                  )}
                </div>
              ...
              {aiResult.hasHazardous && (
                <div className="flex items-start gap-3 p-3 mt-3 rounded-xl"
                     style={{background:'rgba(239,68,68,0.08)',border:'1px solid rgba(239,68,68,0.25)'}}>
                  <AlertTriangle size={16} style={{color:'#F87171',flexShrink:0,marginTop:'2px'}} />
                  <p style={{color:'#FCA5A5',fontSize:'13px'}}>
                    ⚠️ Hazardous waste detected. Requires specialised disposal — do not handle directly.
                  </p>
                </div>
              )}
            </div>
            ...
            <div className="card p-5 mb-5">
              <div className="flex gap-4 flex-wrap">
                ...
                <div style={{flex:1}}>
                  ...
                  <div style={{fontSize:'13px',color:'var(--text-2)'}}>🏛️ Agency: {meta?.agencyEmail || 'To be assigned'}</div>
                </div>
              </div>
            </div>
